...
graph = gs.import_onnx(onnx.load("elan_x4_fixed.onnx"))



# unsupport mod op
mod_13 = [node for node in graph.nodes if node.name=="Mod_2"][0]
mod_13_o = mod_13.o()
...
mod_19 = [node for node in graph.nodes if node.name=="Mod_9"][0]
mod_19_o = mod_19.o()
...
mod_17 = [node for node in graph.nodes if node.name=="Mod_6"][0]
...
mod_23 = [node for node in graph.nodes if node.name=="Mod_13"][0]
...
onnx.save(gs.export_onnx(graph), "elan_x4_sed_fixed.onnx")