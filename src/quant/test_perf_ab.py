...
# test INT8 ptq tensorrt performance
print('====', 'TensorRT int8 PTQ', '====')
logger = trt.Logger(trt.Logger.ERROR)
...
print('****Average diff between trt int8 and pytorch****')
print(tableHead)
timePerInference = time_trt
check0 = check(outputH0,output_pytorch.detach().cpu().numpy(),True,5e-5)
string = "%4d,%6d,%6d,%8.3f,%9.3e,%9.3e,%9.3e,%9.3e,%9.3e,%9.3e,%9.3e"%(batchSize,
...
# test INT8 qat tensorrt performance
print('====', 'TensorRT int8 QAT', '====')
logger = trt.Logger(trt.Logger.ERROR)
with open('elan_x4_qat_3f1.plan', 'rb') as f:
    engine = trt.Runtime(logger).deserialize_cuda_engine(f.read())
if engine == None:
    print("Failed loading engine!")
    exit()
