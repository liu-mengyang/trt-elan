...
trtFile = "./plans/elan_x4_to_fp16_conv.plan"
total = 0
for layer in network:
    if layer.precision != trt.DataType.FLOAT and layer.precision != trt.DataType.HALF:
        continue
...
trtFile = "./plans/elan_x4_to_fp32_matm.plan"
total = 0
for layer in network:
    if layer.precision != trt.DataType.FLOAT and layer.precision != trt.DataType.HALF:
        continue
    if layer.type in [trt.LayerType.MATRIX_MULTIPLY]:
        total += 1
        layer.precision = trt.DataType.FLOAT
        for i in range(layer.num_outputs):
            layer.get_output(i).dtype = trt.DataType.FLOAT
...
trtFile = "./plans/elan_x4_to_fp16_matm.plan"
total = 0
for layer in network:
    if layer.precision != trt.DataType.FLOAT and layer.precision != trt.DataType.HALF:
        continue
